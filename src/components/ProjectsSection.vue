...
<template>
  <section id="projects" class="py-20 relative overflow-hidden">
    <!-- Background Decoration handled by ThemeBackground -->
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="(project, index) in portfolioData.projects" :key="index" 
          ...
          <div class="h-56 overflow-hidden relative">
            <div class="absolute inset-0 bg-gradient-to-t from-[var(--color-dark)] to-transparent opacity-60 z-10 group-hover:opacity-40 transition-opacity duration-500"></div>
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="selectedImage" class="fixed inset-0 z-[60] flex items-center justify-center p-4 md:p-10">
          ...
          <!-- Blurred Backdrop -->
          <div @click="closeLightbox" class="absolute inset-0 bg-[var(--color-surface)]/60 backdrop-blur-2xl transition-all duration-500"></div>

          <!-- Close Button -->
          <button @click="closeLightbox" class="absolute top-6 right-6 p-3 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-[var(--color-text)] transition-all duration-300 hover:rotate-90 z-20 hover:scale-110 shadow-lg border border-white/10">
            <X class="w-6 h-6" />
          </button>
